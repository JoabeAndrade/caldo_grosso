...
export default function DetalheSalada() {
    ...
    const toggleAcompanhamento = (item: string) => {
        if (selectedAcompanhamentos.includes(item)) {
            setSelectedAcompanhamentos(prev => prev.filter(acomp => acomp !== item));
        } else {
            if (selectedAcompanhamentos.length >= 8) {
                Alert.alert('Limite atingido', 'Você pode escolher no máximo 8 acompanhamentos.');
            } else {
            ...
            <View style={styles.section}>
                <View style={styles.headerRow}>
                    ...
                    <View style={styles.badge}>
                        {Object.values(misturasOptions).reduce((acc, curr) => acc + curr, 0) === 2 ? (
                ...
                {acompanhamentosList.map((item, index) => (
                    <TouchableOpacity key={index} style={styles.option} onPress={() => toggleAcompanhamento(item)}>
                        <Text style={styles.optionText}>{item}</Text>
                        <View style={styles.radioSquare}>
                            {selectedAcompanhamentos.includes(item) && <Ionicons name="checkmark" size={16} color="#ef4444" />}
                        </View>
                    </TouchableOpacity>
                ))}
